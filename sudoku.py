...
def sudoku_solutions(board: list[list[int]], n: int) -> Generator[list[list[int]], None, None]:
    for (y, row) in enumerate(board):
        for (x, cell) in enumerate(row):
            if cell == 0:
                columns = [*zip(*board)]
                for i in range(1, n*n + 1):
                    if i not in row and i not in columns[x]:
                        side = n * (x//n)
                        top = n * (y//n)
                        if i not in (board[top+t][side+s] for t in range(n) for s in range(n)):
                            board[y][x] = i
                            yield from sudoku_solutions(board, n)
                            board[y][x] = 0
                return
    yield board
...
for solution in sudoku_solutions(board3, 3):
    if solution == board3_solution:
        print("Test passed for 3x3")
    break
...
for solution in sudoku_solutions(board4, 4):
    if solution == board4_solution:
        print("Test passed for 4x4")
    break
